...
export const Table = prop => {
  ...
  const settingChanges = (coulmnData, filterToggle, selectedPageSize, columnFilter) => {
    setModal(false);
              ...
              {column &&
                column.map((e: any, i: any) => {
                  if (e.visible) {
                    ...
                    return (
                      <Column key={i} filter columnKey={e.field} field={e.field} header={e.header} style={{ width: e.width }} sortable />
                    );
                  }
                })}
            </DataTable>
